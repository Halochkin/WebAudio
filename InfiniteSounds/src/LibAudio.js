...
class AudioFileRegister {
  ...
  static async constant(node, ctx) {
    const constSrc = ctx.createConstantSource();
    constSrc.offset.value = node.body[0];
    ...
    return {graph: node, output: constSrc}
  }
...
InterpreterFunctions.constant = AudioFileRegister.constant;
